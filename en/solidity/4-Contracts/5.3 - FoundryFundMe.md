...
[rpc_endpoints]
baobab = "${WESTEND_RPC_URL}"

# See more config options https://github.com/foundry-rs/foundry/blob/master/crates/config/README.md#all-options
```

5. Add `.env` file and add `WESTEND_RPC_URL`
`WESTEND_RPC_URL` can be obtained on Ankr, Allthatnodes, etc.

```bash
WESTEND_RPC_URL=https://xxxxxx/xxxxx
```
...
```bash
forge script script/DeployFundMe.s.sol --rpc-url $WESTEND_RPC_URL --account $WALLET_NAME --sender $SENDER_ADDRESS --broadcast
```
